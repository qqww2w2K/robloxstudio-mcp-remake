	...
	if (typeIs(propertyValue, "table")) {
		...
		if (typeIs(arr, "table") && (arr as defined[]).size() > 0) {
			...
			if (len === 3) {
				...
				const [success, currentVal] = pcall(() => inst[propertyName]);
				if (success && typeOf(currentVal) === "CFrame") {
					return new CFrame(
						(arr[0] as number) ?? 0,
						(arr[1] as number) ?? 0,
						(arr[2] as number) ?? 0,
		...
		if (tbl._type === "CFrame") {
			...
			if (comps && typeIs(comps, "table") && (comps as defined[]).size() === 12) {
				return new CFrame(
					comps[0],
					comps[1],
					comps[2],
		...
		if (tbl.X !== undefined || tbl.Y !== undefined || tbl.Z !== undefined || tbl.x !== undefined || tbl.y !== undefined || tbl.z !== undefined) {
			...
			return new Vector3(
				(tbl.X ?? tbl.x) as number ?? 0,
				(tbl.Y ?? tbl.y) as number ?? 0,
				(tbl.Z ?? tbl.z) as number ?? 0,
			);
		}

		if (tbl.R !== undefined || tbl.G !== undefined || tbl.B !== undefined || tbl.r !== undefined || tbl.g !== undefined || tbl.b !== undefined) {
			return new Color3(
				(tbl.R ?? tbl.r) as number ?? 0,
				(tbl.G ?? tbl.g) as number ?? 0,
				(tbl.B ?? tbl.b) as number ?? 0,
			);
		}
	}
